...
- **launchd auto-start (`src/launchd.js`).** `daemon install`/`uninstall` write `~/Library/LaunchAgents/com.watchtell.daemon.plist` (label `com.watchtell.daemon`) and load it via `launchctl bootout`+`bootstrap gui/$(id -u)`. `buildPlist()` is pure; `install`/`uninstall` take `opts.platform`/`opts.plistPath`/`opts.launchctlFn` so tests never touch the real `~/Library/LaunchAgents` or `launchctl` — never call real `launchctl` in tests.
