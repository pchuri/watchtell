...
- **rm-vs-run race (`src/store.js` + `src/daemon.js`).** `store.remove` writes a `<id>.removed` **tombstone before** deleting sidecars and leaves it behind; the daemon reaps it (`finalizeRemoval`) and re-checks it around each run and before/after every runtime write (`commitRuntime`, `sweepRemoved`) so an rm overlapping a tick never resurrects a file (state, runtime, or pending alarm) and logs no spam. `newId` skips tombstoned ids so a recycled id is never swept as the removed one. Tests: `test/rm-race.test.js`.
