...
- **ASCII-confusable lint (`src/compile.js` `lintConfusables`).** Runs inside `compile()` after `parse`, BEFORE Keep/hash-bind. Rejects fullwidth forms (U+FF01–FF5E), U+3000, curly quotes, en/em dash, minus sign, NBSP — LLM lookalikes that would ship a broken checker that silently loses its first alarm (issue #13). Whole-file scan; a small explicit list, NOT a non-ASCII ban (Korean grep/message text passes). Lint rejection shares the timeout retry budget (one retry, then fail). Add-time only — daemon/test unchanged.
